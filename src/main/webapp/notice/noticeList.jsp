...
<html lang="ko">
  <head>
    ...
    <script type="text/javascript">
			function noticeInsert() { // GET 방식 호출
				console.log('noticeInsert 호출');
				const in_title = document.getElementById('n_title').value;
				const in_writer = document.getElementById('n_writer').value;
				const in_content = document.getElementById('n_content').value;

				location.href="/notice/noticeInsert?n_title="+in_title+"&n_writer="+in_writer+"&n_content="+in_content;
			}
			function noticeList(){
  ...
  <body>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-3">
		 		<button id="btn_search" class="btn btn-success" onClick="noticeSearch()">검색</button>
		 	</div>
		</div>		
		<!-- 검색기 끝 -->
		<!-- 회원목록 시작 -->
...
%>					
					<tr>
						...
						<td><%=rmap.get("N_NO") %></td>
						<td>
							<a href="javascript:void(0);" onclick="noticeDetail(<%=rmap.get("N_NO") %>);">
								<%=rmap.get("N_TITLE") %>
							</a>
						</td>
	  ...
	  <div class="modal-dialog modal-dialog-centered">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	      	<form id="f_notice" method="post" action="/notice/noticeInsert">
	          ...
	          <div class="form-floating mb-3 mt-3">
	            <textarea rows="5" class="form-control h-25" aria-label="With textarea" id="n_content" name="n_content"></textarea>
						...
						<div class="modal-footer">
							<input type="submit" class="btn btn-warning" data-bs-dismiss="modal" value="저장">
